...
export const Menu = () => {
  ...
  return (
    <div className="relative w-fit" ref={menuRef}>
      ...
      {isOpen && (
        <div
          className="flex flex-col p-[15px] bg-background border-1 border-tertiary-500 absolute right-0 text-nowrap"
        >
          {menuItems.filter(i => i.primary).map(i => (
            <a
              key={i.label}
          ...
          <div className="w-full h-0 border-1 border-secondary-300 my-[10px]" />
          {menuItems.filter(i => !i.primary).map(i => (
            <a
              key={i.label}
